...
typedef enum Map_Result {
	...
	MAP_LIST_NOT_FOUND,				/**<Need to create a new list	*/
	MAP_VALUE_UPDATED,				/**<The value was updated		*/
	MAP_VALUE_INSERTED				/**<Value was created			*/
} Map_Result;
...
typedef void 	(*KeyDestroy)(void* key);
typedef void 	(*ValDestroy)(void* value);
 ...
 * @param[optional] _keyDestroy : pointer to function to destroy keys (if NULL - nothing will be done)
 * @param[optional] _valDestroy : pointer to function to destroy values (if NULL - nothing will be done)
...
/** 
 * @brief Adjust map capacity and rehash all key/value pairs. will be rehashed to a NEW map.
 * User's responsible to destroy old map.
